...
# Conditional demucs import (only if enabled)
try:
    from core.all_whisper_methods.demucs_vl import BACKGROUND_AUDIO_FILE
except ImportError:
    # Fallback when demucs is not available
    BACKGROUND_AUDIO_FILE = "output/audio/background.mp3"
...
def merge_video_audio():
    ...
    # FIX: Only use DUB_AUDIO, don't mix with original English audio
    ...
    # FIX: Use only dubbed audio, remove original English audio completely
    cmd = [
        'ffmpeg', '-y',
        '-i', VIDEO_FILE,      # Input 0: video (ignore its audio)
        '-i', DUB_AUDIO,       # Input 1: Chinese dubbed audio ONLY
        '-filter_complex',
        f'[0:v]scale={target_width}:{target_height}:force_original_aspect_ratio=decrease,'
        f'pad={target_width}:{target_height}:(ow-iw)/2:(oh-ih)/2,'
        f'{subtitle_filter}[v];'
        f'[1:a]volume={dub_volume}[a]'  # Only use dubbed audio, no mixing!
    ]

    # FIX: Map filtered video and dubbed audio (not original audio)

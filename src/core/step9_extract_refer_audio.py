...
try:
    from core.all_whisper_methods.demucs_vl import demucs_main, VOCAL_AUDIO_FILE, RAW_AUDIO_FILE
except ImportError:
    # Fallback when demucs is not available
    VOCAL_AUDIO_FILE = "output/audio/vocal.mp3"
    RAW_AUDIO_FILE = "output/audio/raw.mp3"
    def demucs_main():
        pass  # No-op when demucs disabled
...
def extract_refer_audio_main():
    ...
    # Use VOCAL_AUDIO_FILE if demucs was run, otherwise fall back to RAW_AUDIO_FILE
    audio_file = VOCAL_AUDIO_FILE if os.path.exists(VOCAL_AUDIO_FILE) else RAW_AUDIO_FILE
    rprint(f"[cyan]📁 Using audio file: {audio_file}[/cyan]")
